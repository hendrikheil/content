...
When you specify `extraFields`, TypeScript will automatically infer the types of the returned sections to include those fields. This provides type-safe access to the extra fields:
...
sections[0].author       // ✓ string
sections[0].publishDate  // ✓ string

// TypeScript will error on fields not included
sections[0].category     // ✗ Type error - category not in extraFields
```

This works similarly to how `queryCollection().select()` narrows the return type to only include selected fields.
